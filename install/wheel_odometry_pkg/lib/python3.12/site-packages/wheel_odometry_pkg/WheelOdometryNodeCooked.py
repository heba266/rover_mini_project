...
class WheelOdometryNode(LifecycleNode):
    ...
    def on_configure(self, state):
        ...
        self.timer = self.create_timer(1.0 / self.rate, self.publish_odom)
        self.timer.cancel()
    ...
    def handle_enc(self, enc_msg):
        ...
        current = enc + self.tick_mult * (self.MAX_ENC - self.MIN_ENC)
    
        if self.motion == 0:
            move = current - self.rot_end
            self.total_ticks += move
            self.rot_end = current
